...
cvar_t  sb_hideother     = {"sb_hideother",        "0"};
cvar_t  sb_showonly      = {"sb_showonly",         ""};
...
void Serverinfo_Draw ()
{
    ...
    if (serverinfo_pos == 0)
        memcpy (buf, "\x10נלבשועף\x11", 9); // FIXME: non-ascii chars
    if (serverinfo_pos == 1)
        memcpy (buf + 8, "\x10ףועצועימזן\x11", 12); // FIXME: non-ascii chars
    if (serverinfo_pos == 2)
        memcpy (buf + 19, "\x10ףןץעדוף\x11", 9); // FIXME: non-ascii chars
...
void Filter_Servers(void)
{
    ...
    for (i=0; i < serversn; i++)
    {
        ...
        tmp = ValueForKey(s, "gametype");
        if (strlen(sb_showonly.string) > 0) {
			if (tmp == NULL || strcmp(sb_showonly.string, tmp) != 0)
				continue;
        }

	tmp = ValueForKey(s, "*gamedir");
	if (sb_hideother.value && tmp && strcmp(tmp, com_gamedirfile))
		continue;
