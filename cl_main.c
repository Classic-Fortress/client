...
cvar_t	name					= {"name", "player", CVAR_USERINFO | CVAR_ARCHIVE, CL_OnChange_name_validate};
cvar_t	team					= {"team", "", CVAR_USERINFO | CVAR_ARCHIVE};
cvar_t	topcolor				= {"topcolor","", CVAR_USERINFO | CVAR_ARCHIVE};
cvar_t	bottomcolor				= {"bottomcolor","", CVAR_USERINFO | CVAR_ARCHIVE};
cvar_t	skin					= {"skin", "", CVAR_USERINFO | CVAR_ARCHIVE};
cvar_t	rate					= {"rate", "5760", CVAR_USERINFO | CVAR_ARCHIVE};
			...
			if (!allowremotecmd && (!*localid.string || strcmp(localid.string, s))) 
			{
				...
				else 
				{
					...
					Com_Printf ("Invalid localid on command packet received from local host. "
						"\n|%s| != |%s|\n"
						"You may need to reload your server browser and Classic Fortress.\n",
						s, localid.string);
					Com_Printf ("===========================\n");
					Cvar_Set(&localid, "");
				}
...
void CL_InitLocal (void) 
{
	...
	snprintf(st, sizeof(st), "Classic Fortress %i", REVISION);
...
void CL_UpdateCaption(qbool force)
{
	...
	if (!cl_window_caption.value)
	{
		if (!cls.demoplayback && (cls.state == ca_active))
			snprintf(str, sizeof(str), "Classic Fortress: %s", cls.servername);
		else
			snprintf(str, sizeof(str), "Classic Fortress");
	}
	else
	{
